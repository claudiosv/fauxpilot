...
# Write .env
{
  ...
  echo "GPUS=$(seq 0 $((NUM_GPUS - 1)) | paste -s -d ',' -)"
  echo "API_EXTERNAL_PORT=${API_EXTERNAL_PORT}"
  echo "TRITON_HOST=${TRITON_HOST}"
  echo "TRITON_PORT=${TRITON_PORT}"
} >>.env
...
function fastertransformer_backend() {
  ...
  echo "[9] codegen-16B-nl (32GB total VRAM required; natural language)"
  # Read their choice
...
function python_backend() {
  ...
  *)
  ...
  {
    "MODEL=py-${MODEL}"
    echo "MODEL_DIR=${MODELS_ROOT_DIR}/py-${ORG}-${MODEL}"
