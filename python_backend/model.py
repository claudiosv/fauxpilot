...
class TritonPythonModel:
    def initialize(self, args):
        ...
        org_name = model_config["parameters"].get(
            "org_name", {"string_value": "Salesforce"}
        )["string_value"]
        model_name = (
            org_name + "/" + model_config["parameters"]["model_name"]["string_value"]
        )

        def get_bool(x):
            return model_config["parameters"][x]["string_value"].lower() in [
                "1",
                "true",
            ]
        ...
        self.model = AutoModelForCausalLM.from_pretrained(
            ...
            if is_half
            else ("auto" if torch.cuda.is_available() else torch.float32),
            load_in_8bit=int8,
        ...
        print(
            f"Model {model_name} Loaded. Footprint: {self.model.get_memory_footprint()}"
        )
    ...
    def execute(self, requests):
        ...
        for request in requests:
            ...
            top_k = (
                pb_utils.get_input_tensor_by_name(request, "runtime_top_k")
                .as_numpy()
                .tolist()[0]
            )
            top_p = (
                pb_utils.get_input_tensor_by_name(request, "runtime_top_p")
                .as_numpy()
                .tolist()[0]
            )
            temperature = (
            ...
            for i in range(output_ids.shape[0]):
                sequence_length[i, 0] = torch.sum(
                    output_ids[i, 0] != self.model.config.eos_token_id
                ).item()
