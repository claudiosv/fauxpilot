...
parser.add_argument(
    "--rebase", default=None, help="Path to rebase the model store to (e.g. for Docker)"
)
parser.add_argument(
...
params["vocab_size"] = tokenizer.vocab_size + len(
    tokenizer.get_added_vocab()
)  # round_up(tokenizer.vocab_size, 1024)
...
if args.rebase:
    ...
    rebased_weights_path = os.path.join(
        args.rebase, "fastertransformer", f"{version}", f"{args.num_gpu}-gpu"
    )
else:
