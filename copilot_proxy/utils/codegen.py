...
class CodeGenProxy:
    ...
    def generate(self, data):
        ...
        start_ids = self.PAD_CHAR * np.ones([input_start_ids.shape[0], 1]).astype(
            np_type
        )
        ...
        for i, (text, tokens, lps, g) in enumerate(
            zip(trimmed, output_data, lp_data, gen_len)
        ):
            reason = "length" if max_tokens == g else "stop"
            if lps is not None:
                tokens_str = [
                    self.tokenizer.decode([t])
                    for t in tokens[prompt_len : prompt_len + g]
                ]
                offsets = [len(prompt)] + (
                    np.cumsum([len(t) for t in tokens_str]) + len(prompt)
                ).tolist()[:-1]
                ...
                for ii, t in enumerate(tokens_str):
                    ...
                    while len(fakedict) < num_logprobs:
                        random_token = random.randint(
                            0, self.tokenizer.get_vocab_size() - 1
                        )
